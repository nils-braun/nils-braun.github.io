...
[tsfresh](https://github.com/blue-yonder/tsfresh) is one of them.
...
Of course, also our previously discovered features is there:
...
but in principle you can put in every classification method you can think of her.

## Feature Extraction on real-time data

If we have a historic set of data, we can not extract features automatically and use them to either distinguish between broken and good sensors or train a ML method to help us with this task.
But how do you extract features in near-real time on a stream of sensor data?

Sure, you could just call `tsfresh.extract_features` repeatedly - but do you want to handle the partitioning into chunks of size 100 by yourself?
...
Its basic mode of operations is to build up a computation graph of operations, you want to execute on a stream.
Later, when your events start to flow, `streamz` will call the prepared operations on each of the messages.
This declarative stream processing does not only improve the readability of the code, it also makes general implementations of things like checkointing, windowing or multicore processing (`streamz` uses `dask` under the hood for this) very easy.
...
Every operation you perform on this stream will create a new instance of it (or one of its many subclasses).
...
You should definitely check out the documentation for more information.
...
Let's assume the messages from the sensors are on `json` format, and look roughly like this:
...
def filter_out(x, id_value):
  """Returns true of the message is from sensor with the given id"""
